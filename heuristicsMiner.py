...
OUTPUT_PETRI_NET_PATH = "outputs/heuristics_miner_petri_net.pnml"
OUTPUT_VISUALIZATION_PATH = "outputs/heuristics_miner_model.png"
...
def print_statistics(model):
    ...
    if hasattr(model, "nodes"):
        ...
        activity_freq = sorted(
            [(node.node_name, node.node_occ) for node in model.nodes], key=lambda x: x[1], reverse=True
        )[:10]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Heuristics Miner wrapper using pm4py")
    parser.add_argument("--input", "-i", default=INPUT_PATH, help="Path to filtered CSV file")
    parser.add_argument("--out", "-o", default="outputs", help="Output directory for PNML/PNG")
    ...
    net, initial_marking, final_marking = discover_model_heuristics(
        log_df, dependency_threshold=0.8, and_threshold=0.65, loop_two_threshold=0.5
    )
